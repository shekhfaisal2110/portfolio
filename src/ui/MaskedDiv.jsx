...
// Define available mask types and their paths
const svgPaths = {
  "type-1": {
...
const MaskedDiv = ({
  ...
  // Dynamically calculate aspect ratio based on mask type
  useEffect(() => {
    const mask = svgPaths[maskType];
    setAspectRatio(mask.width / mask.height);
  }, [maskType]);
  ...
  const renderImages = React.Children.map(children, (child) => (
    <div className="image-container" style={containerStyle}>
      {React.cloneElement(child, {
        className: `w-full h-full object-cover object-center hover:scale-105 transition-all duration-300 ${child.props.className || ""}`,
      })}
    </div>
  ));

  return <section className={`relative ${className}`}>{renderImages}</section>;
};
