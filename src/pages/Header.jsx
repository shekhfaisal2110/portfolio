...
const Header = () => {
  ...
  const SkeletonHeader = () => (
    <header className="fixed top-0 w-full z-50 bg-gray-950/90 backdrop-blur-xl border-b border-white/10 h-20">
      <nav className="max-w-7xl mx-auto px-6 flex items-center justify-between h-full">
        {/* Logo Skeleton */}
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-gradient-to-r from-cyan-500/20 to-blue-600/20 rounded-full animate-pulse"></div>
          <div className="h-8 w-32 bg-gradient-to-r from-cyan-500/20 to-purple-600/20 rounded animate-pulse"></div>
        </div>

        {/* Desktop Nav Skeleton (hidden on mobile) */}
        <div className="hidden lg:flex gap-2">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="w-20 h-10 bg-white/10 rounded-lg animate-pulse"></div>
          ))}
        </div>

        {/* Mobile Menu Button Skeleton */}
        <div className="lg:hidden w-12 h-12 bg-white/10 rounded-xl animate-pulse"></div>

        {/* Desktop CTA Skeleton */}
        <div className="hidden lg:block w-28 h-12 bg-gradient-to-r from-cyan-500/30 to-blue-600/30 rounded-xl animate-pulse"></div>
      </nav>
    </header>
  );
  ...
  if (isLoading) {
    return (
      <>
        <ParticleTrail />
        <SkeletonHeader />
        {/* Sidebar remains hidden during loading */}
      </>
    );
  }
        ...
        <nav className="max-w-7xl mx-auto px-6 flex items-center justify-between h-20 relative">
          ...
          >
            <div className="relative">
              ...
              <div className="relative w-14 h-14 bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full flex items-center justify-center shadow-lg transform transition-all duration-300 group-hover:scale-110 group-hover:rotate-6">
                <img src={logoImg} alt="Developer" className="w-10 h-10 rounded-full object-cover" />
              </div>
          ...
          {!sidebarOpen && (
            ...
            >
              ...
              <div
                className="relative w-12 h-12 bg-white/10 rounded-xl border border-white/20 flex items-center justify-center text-white text-xl transition-all duration-300 hover:scale-110 hover:bg-white/20"
          ...
          {sidebarOpen && (
            ...
            >
              ...
              <div
                className="relative w-10 h-10 bg-white/10 rounded-lg border border-white/20 flex items-center justify-center text-white transition-all duration-300 hover:scale-110 hover:bg-white/20"
                style={{ backdropFilter: "blur(16px)" }}
              >
                <X className="text-lg" />
              </div>
          ...
          >
            ...
            <div className="mt-8 p-6 relative">
              ...
              <div className="relative bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6 text-center">
                ...
                <button onClick={() => handleTabSelect("/contact")} className="w-full group relative">
                  <div className="absolute -inset-1 bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 rounded-xl blur opacity-0 group-hover:opacity-100 transition duration-300" />
